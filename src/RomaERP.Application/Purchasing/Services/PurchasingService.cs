...
public class PurchasingService : IPurchasingService
{
    ...
    /// <summary>Records what physically arrived from a vendor, item by item, for the Restaurant "Purchase
    /// Receiving" screen — an internal-control record separate from the vendor's actual invoice, which is
    /// still entered by its total on the accounting Purchase Invoices screen. Deliberately does NOT post a
    /// journal entry or touch the vendor's AP balance, so the same delivery is never counted as a liability
    /// twice; it only updates item quantity and weighted-average cost.</summary>
    public async Task<InventoryReceiptDto> ReceiveInventoryPurchaseAsync(ReceiveInventoryPurchaseDto dto, CancellationToken ct = default)
    {
        if (dto.Lines.Count == 0)
            throw new ValidationAppException("لازم يكون في صنف واحد على الأقل.");
        ...
        var resultLines = new List<InventoryReceiptLineDto>();
        ...
        foreach (var (dtoLine, index) in dto.Lines.Select((l, i) => (l, i)))
        {
            ...
            _context.StockMovements.Add(new StockMovement
            {
                ...
                TotalCost = totalCost,
                Reference = null,
                Description = $"استلام مشتريات (رقابة داخلية) - {vendor.NameAr}" + (string.IsNullOrWhiteSpace(dto.Notes) ? "" : $" — {dto.Notes}"),
                JournalEntryId = null
            });

            resultLines.Add(new InventoryReceiptLineDto
            {
                ItemId = item.Id,
                ItemCode = item.Code,
        ...
        {
            VendorName = vendor.NameAr,
            WarehouseName = warehouse.NameAr,
            ReceiptDate = dto.ReceiptDate,
            TotalCost = resultLines.Sum(l => l.Quantity * l.UnitCost),
            Lines = resultLines
        };
    }
