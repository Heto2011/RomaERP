...
/// next invoice, so it must be returned even though it's also embedded inside SignedXml's QR code.</summary>
public record ZatcaSigningResult(string SignedXml, string InvoiceHash);
...
public class MockZatcaDocumentSigner : IZatcaDocumentSigner
{
    public Task<ZatcaSigningResult> SignInvoiceXmlAsync(string unsignedInvoiceXml, CompanySettings settings, CancellationToken ct = default)
    {
        ...
        return Task.FromResult(new ZatcaSigningResult(signed, hash));
    }
}
