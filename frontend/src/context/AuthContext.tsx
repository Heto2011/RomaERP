...
interface AuthContextValue {
  ...
  loginWithToken: (companyCode: string, token: string, email: string, fullName: string, roles: string[], modules?: string[], productScope?: ProductScope) => void;
  logout: () => void;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function login(companyCode: string, email: string, password: string) {
    ...
    const authUser = { email: data.email, fullName: data.fullName, roles: data.roles, modules: data.modules, productScope: data.productScope };
  ...
  function loginWithToken(companyCode: string, token: string, email: string, fullName: string, roles: string[], modules: string[] = [], productScope: ProductScope = ProductScope.Full) {
    localStorage.setItem("companyCode", companyCode);
    localStorage.setItem("token", token);
    const authUser = { email, fullName, roles, modules, productScope };
    localStorage.setItem("user", JSON.stringify(authUser));
    setUser(authUser);
  }
